...
km = KeyManager()
km.ensure_keys()  # creates ~/.omniroute/keys/ if not present
...
KeyManager.ensure_keys()
    │
    ├── ~/.omniroute/keys/ed25519_private.key  (mode 0600)
    └── ~/.omniroute/keys/ed25519_public.key   (mode 0644)
...
|------|---------|-------------|
| `~/.omniroute/keys/` | Ed25519 private + public keys | `600` (private), `644` (public) |
| `~/.omniroute/certs/` | TLS cert + key for ASGI proxy | `600` (key), `644` (cert) |
