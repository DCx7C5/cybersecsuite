...
interface Investigation { id: string; title: string; status: string; severity: string; created_at: string; description?: string }
interface InvestigationsData { investigations?: Investigation[] }

function sev(s: string): 'err' | 'warn' | 'info' | 'muted' {
...
export default function InvestigationsPanel() {
  const { data, isLoading, error } = useApiQuery<InvestigationsData>(['investigations'], '/api/investigations')
  const [showModal, setShowModal] = useState(false)
  const [editItem, setEditItem] = useState<Investigation | null>(null)
  ...
  const saveMut = RQ.useMutation({
    mutationFn: () => {
      const url = editItem ? `/api/investigations/${editItem.id}` : '/api/investigations'
      const method = editItem ? 'PATCH' : 'POST'
      return fetchApi(url, { method, body: JSON.stringify(form) })
    },
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ['investigations'] }); setShowModal(false); setEditItem(null) }
  })

  const deleteMut = RQ.useMutation({
    mutationFn: (id: string) => fetchApi(`/api/investigations/${id}`, { method: 'DELETE' }),
  ...
  const columns: ColumnDef<Investigation>[] = [
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => <Badge variant="info">{getValue<string>()}</Badge> },
    { accessorKey: 'severity', header: 'Severity', cell: ({ getValue }) => <Badge variant={sev(getValue<string>())}>{getValue<string>()}</Badge> },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <Card title="Investigations" actions={<Button onClick={() => { setEditItem(null); setForm({ title: '', description: '', status: 'open', severity: 'medium' }); setShowModal(true) }}>+ New</Button>}>
        <Table data={investigations} columns={columns} />
      </Card>
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Edit Investigation' : 'New Investigation'}>
