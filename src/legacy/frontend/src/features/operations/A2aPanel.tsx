...
export default function A2aPanel() {
  const { data: status } = useApiQuery<A2aStatus>(['google_a2a-status'], '/api/a2a')
  const { data, isLoading, error } = useApiQuery<A2aData>(['google_a2a-tasks'], '/api/a2a/tasks')
  ...
  const createMut = RQ.useMutation({
    ...
    onSuccess: () => { void qc.invalidateQueries({ queryKey: ['google_a2a-tasks'] }) }
  })
