...
test.describe('WCAG 2.1 Level AA Accessibility Audit', () => {
  ...
  test('2. skills detail page should be accessible', async ({ page, browserName }) => {
    reportData.browser = browserName;
    
    // Navigate to first marketplace page and look for a skills link
    await page.goto('/');
    await page.waitForLoadState('networkidle').catch(() => null);

    // Try to find and click a skills link, or navigate directly
    const skillLink = page.locator('[data-testid="skills-card"], a[href*="/skills"], a[href*="/skills"]').first();
    const href = await skillLink.getAttribute('href').catch(() => null);

    if (href) {
      await page.goto(href);
      await scanPage(page, 'Skill Detail Page', href, '[data-testid="skills-detail"], main');
    } else {
      // Fallback to testing the marketplace page if no skills links found
      console.log('⏭️  No skills detail page found, testing marketplace instead');
      await scanPage(page, 'Skill Detail Page (fallback)', '/', 'main');
    }
  });

  // Test 3: MCP Tool Documentation
  test('3. MCP tools documentation should be accessible', async ({ page, browserName }) => {
    reportData.browser = browserName;
