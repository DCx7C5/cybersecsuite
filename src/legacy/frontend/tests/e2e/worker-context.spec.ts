 ...
 * - Display worker architecture and scopes info
...
test.describe("Worker Context Information Display", () => {
  ...
  test("should display worker scopes information", async ({ page }) => {
    const scopeElement = page.locator('[data-testid="worker-scopes"]');
    if (await scopeElement.isVisible()) {
      const scopeText = await scopeElement.textContent();
      expect(scopeText).toBeTruthy();
      // Should contain scopes details
