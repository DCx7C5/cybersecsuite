...
memories/       SDK memory tool root (/memories/* paths)
```
...
| You say | Action |
|---------|--------|
| "scaffold vault for [purpose]" | `vault_scaffold` MCP tool |
| "ingest [file or url]" | `vault_ingest` MCP tool |
| "what do you know about X" | `vault_query` MCP tool |
| "lint vault" | `vault_lint` MCP tool |
| "vault status" | `vault_status` MCP tool |
| "/canvas [type]" | `canvas_create` MCP tool |
