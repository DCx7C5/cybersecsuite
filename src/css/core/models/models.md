...
**Tracking rule**: `.plan/session.db` is authoritative for todo status. This document owns the executable model-catalog specification.
...
✅ ModelPricing value type with token-based cost calculation; any touched
implementation must follow the current `msgspec.Struct` rule.
...
**Details**: Query `.plan/session.db` for current status; retain model implementation detail in this local document.
