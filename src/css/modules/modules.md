...
- `vector_rag` is planned to split into core-owned retrieval subsystems under `src/css/core/vector_rag/` and `src/css/core/graph_rag/`; the current `src/css/modules/vector_rag/` package is legacy migration surface until Phase 20 hybrid-retrieval work lands.
