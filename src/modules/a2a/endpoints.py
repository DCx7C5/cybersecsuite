...
# App-scoped routes — mounted at /a2a/*
router = APIRouter(prefix="/a2a", tags=["a2a"])

# Root-level routes — mounted without prefix (e.g. /.well-known/agent.json)
...
async def agent_card_handler(request: Request) -> JSONResponse:
    """Serve the agent identity card (A2A discovery endpoint)."""
    card = _get_agent_card(request)
    if card is None:
        return JSONResponse(status_code=404, content={"detail": "No agent card configured"})
...
def init_a2a_endpoints(
    ...
    Args:
        ...
        agent_card: Optional agent identity card for ``/.well-known/agent.json``.
    """
