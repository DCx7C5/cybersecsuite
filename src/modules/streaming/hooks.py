"""agent.hooks — SDK hook callbacks for CyberSecSuite.
...
async def security_hook(input_data: dict[str, Any], **_: Any) -> dict[str, Any]:
    ...
    for pattern in _BLOCKED_PATTERNS:
        if pattern.search(cmd):
            logger.warning("security_hook BLOCKED: tool=%s pattern=%s", tool_name, pattern.pattern)
            return {
                "action": "block",
...
async def audit_hook(
    ...
    **_: Any,
    ...
    """PreToolUse — log every tool call to structured audit trail."""
    tool_name = input_data.get("tool_name", "")
    agent_id = input_data.get("agent_id", "")
    logger.info("TOOL_CALL tool=%s agent=%s id=%s", tool_name, agent_id, tool_use_id)
...
async def ioc_hook(
    ...
    **_: Any,
    ...
    """PostToolUse — extract IPs, domains, hashes from tool output."""
    if not output_data:
        return {}
