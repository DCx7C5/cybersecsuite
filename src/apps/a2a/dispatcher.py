...
class MessageDispatcher:
    ...
    def register(self, agent_id: str, handler: Callable[[Message], Awaitable[None]]) -> None:
        """Register an async handler for a given agent ID."""
    ...
    async def _send_direct(self, msg: Message) -> None:
        channel = f"channel:agent:{msg.to_id}"
    ...
    async def start_listener(self, entity_id: str) -> None:
        ...
        Each agent/role should run this in its own task::

            asyncio.create_task(dispatcher.start_listener("agent-1"))
        ...
        await pubsub.subscribe(f"channel:agent:{entity_id}")
        ...
        try:
            ...
            await pubsub.unsubscribe(f"channel:agent:{entity_id}")
