...
class A2ACommunicator:
    ...
    ``agent_id`` is the canonical agent identifier (e.g. ``"research:analyst"``).
    ...
    async def send_agent_message(
        ...
        """Send an agent response message back into a task."""
        if parts is None:
        ...
        if task:
            ...
            log.debug("A2A task %r: agent message appended (%d parts)", task_id, len(parts))
...
class A2ACommunicationGroup:
    """A2A protocol group for multi-agent collaboration.
    ...
    ``members`` stores agent IDs (e.g. ``["research:orchestrator", "research:analyst"]``).
    """
    ...
    async def add_member(self, agent_id: str) -> None:
        """Add an agent to the team (idempotent)."""
    ...
    async def remove_member(self, agent_id: str) -> None:
        """Remove an agent from the team (no-op if not present)."""
        try:
    ...
    async def broadcast_task(
        ...
        Internally routes via Redis; externally seen as multi-agent task execution.
        """
