...
export async function loadQolControls() {
    try {
        const scopeEl = _$('qol-scope-select');
        ...
        if (el)
            el.textContent = `Error loading QoL settings: ${e}`;
    }
}
function _renderScope(scope) {
    const el = _$('qol-scope-select');
    if (el)
        el.value = scope;
}
function _renderToggles(settings) {
...
function _renderPresets(presets, activeToggles) {
    const container = _$('qol-presets');
    if (!container)
        return;
    const names = Object.keys(presets);
    if (!names.length) {
        container.innerHTML = '<span style="color:var(--text-muted);font-size:12px">No presets available.</span>';
        return;
    }
...
export async function qolToggle(toggle, enabled) {
    try {
        const scopeEl = _$('qol-scope-select');
        const scope = scopeEl?.value ?? 'session';
        await fetch('/api/qol', {
        ...
        console.error('qolToggle:', e);
    }
}
export async function qolApplyPreset(name) {
    try {
        const scopeEl = _$('qol-scope-select');
        const scope = scopeEl?.value ?? 'session';
        await fetch('/api/qol', {
        ...
        console.error('qolApplyPreset:', e);
    }
}
export async function qolReset() {
    try {
        const scopeEl = _$('qol-scope-select');
        const scope = scopeEl?.value ?? 'session';
