"""
Scope utilities for 5-level scope management (T045).
...
class ScopeContext:
    """Context object for scope resolution."""
    ...
    def _validate(self) -> None:
        ...
        # Higher scopes should have lower scope attributes
        if self.scope_level == ScopeLevel.GLOBAL:
            ...
            if not self.runtime_id:
                raise ValueError("RUNTIME scope requires runtime_id")
            if not self.worktree_path:
                raise ValueError("RUNTIME scope requires worktree_path")
        elif self.scope_level == ScopeLevel.SESSION:
            if not self.session_id:
                raise ValueError("SESSION scope requires session_id")
            if not self.project_id:
                raise ValueError("SESSION scope requires project_id")
...
def resolve_scope_path(
    ...
    """Resolve the filesystem path for a given scope.
    ...
    Returns:
        Resolved filesystem path for the scope

    Raises:
        ValueError: If context is missing required fields for the scope level
    """
    if isinstance(scope_level, str):
        try:
    ...
    if scope_level == ScopeLevel.GLOBAL:
        ...
        if not context.project_id:
            raise ValueError("PROJECT scope requires project_id in context")
        ...
        if not context.runtime_id or not context.session_id:
            raise ValueError("RUNTIME scope requires runtime_id and session_id in context")
        ...
        if not context.project_id or not context.session_id:
            raise ValueError("SESSION scope requires project_id and session_id in context")
        return str(
...
def load_scope_config(
    ...
    """Load scope configuration from filesystem.

    Looks for a config file at the resolved scope path. If not found,
    ...
    try:
        ...
        if config_path.exists():
            with open(config_path, "r") as f:
                config = json.load(f)
                logger.debug(f"Loaded scope config from {config_path}")
                return config
        else:
        ...
        logger.error(f"Error loading scope config: {exc}")
        return _get_default_scope_config(scope_level, context)
...
def _get_default_scope_config(
    ...
    """Get default configuration for a scope level."""
    base_config = {
    ...
    # Set default permissions based on scope level
...
def check_scope_permission(
    ...
    """Check if user has permission for an action on a resource within a scope.

    Permission hierarchy:
        - Global scope users can access all scopes
        - App scope users can access app and lower scopes
        - Project scope users can access their project and lower scopes
        - Runtime/Session scope users can access only their scope
    ...
    try:
        ...
        logger.error(f"Error loading scope config for permission check: {exc}")
        return False
    ...
    if allowed_users and user_id not in allowed_users:
        ...
        if allowed_users:
            logger.debug(f"User {user_id} not in allowed_users list for {scope.scope_level} scope")
            return False
...
def check_scope_hierarchy(
    ...
    """Check if target scope is within source scope hierarchy.

    Hierarchy (inclusive):
        GLOBAL > APP > PROJECT > RUNTIME > SESSION

    Args:
        source_scope: Higher scope
        target_scope: Potentially lower scope
...
def validate_scope_fields(
    ...
    """Validate scope fields for model save operations.
    ...
    if scope_level == ScopeLevel.GLOBAL:
        ...
        if not runtime_id:
            return False, "RUNTIME scope requires runtime_id"
        if not worktree_path:
            return False, "RUNTIME scope requires worktree_path"
    elif scope_level == ScopeLevel.SESSION:
        if not session_id:
            return False, "SESSION scope requires session_id"
        if not project_id:
            return False, "SESSION scope requires project_id"
...
def get_scoped_queryset(
    ...
    """Build a queryset filtered by scope.

    This is a helper for building filtered queries at different scope levels.
