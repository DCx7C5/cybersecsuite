...
def list_resources(base_url, api_key):
    """List available plugins, groups, and sending profiles."""
    client = GoPhishClient(base_url, api_key)
    return {
        "groups": [{"id": g["id"], "name": g["name"], "targets": len(g.get("targets", []))}
                   for g in client.get_groups()],
        "plugins": [{"id": t["id"], "name": t["name"]} for t in client.get_templates()],
...
def main():
    ...
    sub.add_parser("resources", help="List plugins, groups, profiles")
