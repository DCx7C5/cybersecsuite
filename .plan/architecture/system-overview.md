...
- `core/rag_vector/` provides vector retrieval plus hybrid/auto orchestration for `ContextAssembler` and agent execution.
- `core/rag_graph/` provides the graph retrieval implementation behind `graph` and `hybrid` modes.
- `modules/mitre/` and `modules/threat_intel/` keep canonical cybersec domain data while projecting graph-native structures into `core/rag_graph/`.
