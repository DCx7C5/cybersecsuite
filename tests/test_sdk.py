...
def temp_workspace(tmp_path):
    ...
    (tmp_path / ".cybersec" / "sessions").mkdir(parents=True)
    return tmp_path
...
async def test_session_context_manager_creates_dir(temp_workspace, monkeypatch):
    ...
    with sdk.session("test-hunt") as session:
        ...
        session_dir = sdk._sessions_dir / "test-hunt"
        assert session_dir.exists()
        assert sdk._active_session == "test-hunt"

    manifest_path = sdk._sessions_dir / "test-hunt" / "session-manifest.json"
...
async def test_last_session_returns_pointer(temp_workspace, monkeypatch):
    ...
    sdk = CyberSecSuiteSDK()
    last_session_file = sdk._sessions_dir / ".last_session"
...
async def test_last_session_no_pointer_raises(temp_workspace, monkeypatch):
    ...
    sdk = CyberSecSuiteSDK()
    last_file = sdk._sessions_dir / ".last_session"
    if last_file.exists():
        last_file.unlink()
...
async def test_set_pov_writes_context(temp_workspace, monkeypatch):
    ...
    sdk = CyberSecSuiteSDK()
    with sdk.session("test-session"):
        sdk.set_pov("red")

        ctx_file = sdk._sessions_dir / "test-session" / "context.yaml"
